...
const reqHandler = async (req, res) => {
  ...
  switch (method) {
    ...
    case "POST":
      return authMiddleware(req, res, true, createCat);
    case "UPDATE":
      return authMiddleware(req, res, true, updateCat);
    case "DELETE":
      return authMiddleware(req, res, true, deleteCat);
    default:
      return res.status(422).send({ message: "req_method_not_supported" });
  }
};
